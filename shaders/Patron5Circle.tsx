...
void main() {
  ...
  // todo: check range [0, 0.3]?
  // float combo = step(0.3, distance(vUv, vec2(0.5)));
  // todo: check range [-0.1, 0.3]?
  // float combo = step(0.3, distance(vUv, vec2(0.5)) - 0.1);
  // todo: check range [-0.3, 0.01]?
  // float combo = step(0.01, abs(distance(vUv, vec2(0.5)) - 0.3));

  float combo = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - 0.3));
...
void main() {
  ...
  float combo = step(0.01, abs(distance(wave, vec2(0.5)) - 0.3));
...
void main() {
  ...
  vec2 wave = vec2(
    vUv.x + sin(vUv.y * utime) * uleverX,
    vUv.y + sin(vUv.x * utime) * uleverX
  );
