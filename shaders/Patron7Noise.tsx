...
void main() {
  ...
  float combo = 1.0 - step(0.1, cnoise(vUv * utime));
...
void main() {
  // float combo = cnoise(vUv * utime);
  float combo = 1.0 - step(0.1,  abs(cnoise(vUv * utime)));
...
void main() {
  // float combo = cnoise(vUv * utime);
  float combo = step(uleverX,  sin(cnoise(vUv * utime) * utime));

  // black 0,0,0 ,  white 1,1,1
  vec3 black = vec3(0.0);
  vec3 colors = vec3(vUv, 1.0);
  vec3 mezcla = mix(black, colors, combo);
  //gl_FragColor = vec4(colors, 1.0);
  gl_FragColor = vec4(mezcla, 1.0);
